...
// Later, after verifying your own domain in Resend,
// change EMAIL_FROM to something like:
// DreamStay <no-reply@yourdomain.com>
const FROM_EMAIL =
    process.env.EMAIL_FROM || "DreamStay <onboarding@resend.dev>";
        ...
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #eee; border-radius: 12px;">
                ...
                <div style="background: #f8f4f1; text-align: center; padding: 20px; border-radius: 10px; margin: 20px 0;">
                    <span style="font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #212529;">
                        ${otp}
                    </span>
                </div>
...
function emailShell(title, bodyHtml) {
    return `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #eee; border-radius: 12px;">
            ...
            <h2 style="color: #D85A30; margin-bottom: 4px;">
                DreamStay
            </h2>

            <h3 style="color: #212529; margin-top: 0;">
                ${title}
            </h3>
            ...
            <p style="color: #999; font-size: 0.8rem; margin-top: 24px;">
                DreamStay · This is an automated email.
            </p>
...
async function sendBookingConfirmationEmail(
    toEmail,
    username,
    { listingTitle, checkIn, checkOut, guests, grandTotal }
) {
    console.log("📧 BOOKING EMAIL FUNCTION CALLED:", toEmail);
    const html = emailShell(
        ...
        `
            ...
            <p style="color: #555;">
                You can view or manage this booking anytime from your DreamStay account.
            </p>
        `
    );
...
async function sendNewBookingHostEmail(
    toEmail,
    hostUsername,
    { listingTitle, guestUsername, checkIn, checkOut, grandTotal }
) {
    const html = emailShell(
...
async function sendCancellationEmail(
    toEmail,
    username,
    { listingTitle, refundAmount, refundStatus, isHost }
) {
    const bodyForGuest = `
        <p style="color: #555;">
            Hi ${username},
    ...
    const bodyForHost = `
        ...
        <p style="color: #555;">
            A guest cancelled their booking for
            <strong>${listingTitle}</strong>.
        </p>
    `;
    ...
    const html = emailShell(
        "Booking Cancelled",
        isHost ? bodyForHost : bodyForGuest
    );
